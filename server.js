...
const WASTELAND2_STATUS_CACHE_TTL_MS = clampInteger(process.env.WASTELAND2_STATUS_CACHE_TTL_MS, 60000, 1000, 300000);

const WASTELAND2_STEAM_APP_ID = 240760;
const WASTELAND2_STEAM_URL = "https://store.steampowered.com/app/240760/Wasteland_2_Directors_Cut/";
const WASTELAND2_GOG_URL = "https://www.gog.com/en/game/wasteland_2_directors_cut_digital_classic_edition";
const WASTELAND2_DISCUSSIONS_URL = "https://steamcommunity.com/app/240760/discussions/";
const WASTELAND2_GUIDES_URL = "https://steamcommunity.com/app/240760/guides/";
const WASTELAND2_NEWS_URL = "https://store.steampowered.com/news/app/240760";
const WASTELAND2_WIKI_URL = "https://en.wikipedia.org/wiki/Wasteland_2";
const WASTELAND2_OFFICIAL_URL = "https://wasteland.inxile-entertainment.com/";
...
async function fetchJson(url, sourceLabel) {
  ...
  try {
    const response = await fetch(url, {
      headers: {
        "User-Agent": "falloutfanatics-wasteland2-api/1.0",
...
async function fetchSteamCurrentPlayers(appId = WASTELAND2_STEAM_APP_ID) {
  let lastError = null;
...
async function getWasteland2StatusPayload() {
  const cacheKey = "wasteland2:status";
  const cached = getCachedPayload(cacheKey, WASTELAND2_STATUS_CACHE_TTL_MS);
  ...
  const [
    ...
    fetchPageStatus(WASTELAND2_DISCUSSIONS_URL, "Wasteland 2 Steam discussions page"),
    fetchPageStatus(WASTELAND2_GOG_URL, "Wasteland 2 GOG page"),
    fetchPageStatus(WASTELAND2_GUIDES_URL, "Wasteland 2 guides page"),
    fetchPageStatus(WASTELAND2_NEWS_URL, "Wasteland 2 news page"),
    fetchPageStatus(WASTELAND2_WIKI_URL, "Wasteland 2 wiki page"),
    fetchPageStatus(WASTELAND2_OFFICIAL_URL, "Wasteland 2 official page")
  ]);
  ...
  const items = [
    {
      ...
      httpStatus: null,
      url: WASTELAND2_STEAM_URL,
      title: "Wasteland 2: Director's Cut on Steam",
      description: "Текущий онлайн Wasteland 2 в Steam. Это число игроков в PC Steam, а не какой-либо серверный онлайн.",
    ...
    },
    {
      ...
      url: discussionsPage?.url || WASTELAND2_DISCUSSIONS_URL,
      title: "Steam Community :: Wasteland 2 Discussions",
      description: "Раздел обсуждений Wasteland 2 в Steam Community с вопросами, ответами и полезными темами игроков.",
    ...
    },
    {
      ...
      url: gogPage?.url || WASTELAND2_GOG_URL,
      title: "Wasteland 2 Director's Cut on GOG",
      description: "Страница Wasteland 2: Director's Cut в магазине GOG с DRM-free версией игры.",
    ...
    },
    {
      ...
      url: guidesPage?.url || WASTELAND2_GUIDES_URL,
      title: "Steam Community :: Wasteland 2",
      description: "Подборка пользовательских гайдов и советов по Wasteland 2 в Steam Community.",
    ...
    },
    {
      ...
      url: newsPage?.url || WASTELAND2_NEWS_URL,
      title: "Wasteland 2 - Steam News Hub",
      description: "Лента новостей и обновлений Wasteland 2 в Steam.",
    ...
    },
    {
      ...
      url: wikiPage?.url || WASTELAND2_WIKI_URL,
      title: "Wasteland 2 - Wikipedia",
      description: "Энциклопедическая страница Wasteland 2 с общей информацией об игре.",
    ...
    },
    {
      ...
      url: officialPage?.url || WASTELAND2_OFFICIAL_URL,
      title: "The Wasteland Chronicles",
      description: "Официальная страница серии Wasteland от inXile с материалами и навигацией по Wasteland 2.",
  ...
  const payload = {
    service: "falloutfanatics-wasteland2-api",
    ...
    disclaimer: "Wasteland 2 не является онлайн-игрой с публичным списком серверов. Эта страница показывает реальный Steam онлайн и доступность ключевых публичных страниц по игре.",
    items
  };
...
app.get("/", (_req, res) => {
  res.type("text/plain").send("FalloutFanatics Wasteland 2 API is running.");
});
...
app.get("/api/wasteland-2-status", async (_req, res) => {
  try {
    ...
    res.status(502).json({
      ok: false,
      error: "WASTELAND2_STATUS_FETCH_FAILED",
      message: error?.message || "Unable to build Wasteland 2 status payload.",
      fetchedAt: new Date().toISOString()
    });
  }
});
...
app.listen(PORT, HOST, () => {
  console.log(`Wasteland 2 API listening on http://${HOST}:${PORT}`);
});
